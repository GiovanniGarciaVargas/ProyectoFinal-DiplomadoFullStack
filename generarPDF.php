...
try {
    ...
    // Consultar incidencias según la modalidad
    $sql = "SELECT i.fk_eCodAsignacion, i.tDescripcionIncidencia, CONCAT(u.tNombreUsuario, ' ', u.tApellidoPaterno, ' ', u.tApellidoMaterno) AS reportado_por
            ...
            WHERE i.tModalidadIncidencia = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $modalidad);
    $stmt->execute();
    $result = $stmt->get_result();
    if ($result->num_rows == 0) {
        throw new Exception("No se encontraron incidencias para la modalidad especificada.");
    }
    ...
    // Fecha del reporte
    $fecha = date('d') . ' de ' . obtenerNombreMes(date('m')) . ' del ' . date('Y');
    $pdf->Cell(0, 10, utf8_decode('Manzanillo, Colima a ' . $fecha), 0, 1, 'R');
    $pdf->Ln(10);

    // Texto de presentación ampliado
    $pdf->MultiCell(0, 5, utf8_decode('A Quien Corresponda,

A continuación, se presenta el reporte de incidencias para la modalidad '), 0, 'J');
    
    // Modalidad seleccionada en rojo
    $pdf->SetTextColor(255, 0, 0); // Rojo
    $pdf->Cell(0, 10, utf8_decode($modalidad), 0, 1, 'C');
    $pdf->SetTextColor(0); // Restaurar color negro
    
    $pdf->MultiCell(0, 10, utf8_decode('Este reporte tiene como finalidad proporcionar una visión detallada de las incidencias técnicas registradas en la modalidad especificada. 
'), 0, 'J');
    $pdf->Ln(10);
...
Por favor, no dude en ponerse en contacto para cualquier consulta adicional o para coordinar la resolución de los problemas técnicos reportados.'), 0, 'J');
    ...
    // Salida del PDF
    $pdf->Output('I', 'Reporte_Incidencias_' . $modalidad . '.pdf');
} catch (Exception $e) {
